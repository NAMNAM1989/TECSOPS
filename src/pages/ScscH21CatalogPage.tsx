...
export function ScscH21CatalogPage({ onBack }: Props) {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const list = await fetchScscH21Goods({ activeOnly: false });
      setItems(list);
    } catch (e) {
  ...
  const handleSave = async (row: Draft) => {
    ...
    try {
      if (row._isNew) {
        ...
        const created = await createScscH21Goods(payload);
        setItems((prev) => prev.map((r) => (r.id === row.id ? created : r)));
        toast.success("Đã thêm mặt hàng");
      } else {
        const saved = await updateScscH21Goods(row.id, row);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteScscH21Goods(row.id);
      setItems((prev) => prev.filter((x) => x.id !== row.id));
  ...
  return (
    <div className="min-h-screen bg-ui-background" data-testid="scsc-h21-catalog-page">
      ...
      <main className="mx-auto max-w-7xl space-y-3 px-4 py-4">
        ...
        {loading ? (
          ...
          <div className={`${OPS.empty} rounded-2xl py-10 text-sm`}>
            {items.length === 0
              ? "Chưa có mặt hàng. Bấm «+ Thêm» hoặc nhập Excel DATA SC."
      ...
      <ConfirmDialog
        ...
        title="Xóa mặt hàng?"
        message="Xóa khỏi danh mục H21 SCSC. Các dòng invoice đã gắn lô vẫn giữ snapshot."
        confirmLabel="Xóa"
        danger
