...
type Draft = TcsH21StampId & { _isNew?: boolean };
...
export function TcsH21ShipperSection() {
  const toast = useToast();
  const [items, setItems] = useState<TcsH21StampId[]>([]);
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await fetchTcsH21Stamps());
    } catch (e) {
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await deleteTcsH21Stamp(deleteId);
