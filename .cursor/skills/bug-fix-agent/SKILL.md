...
`git status` trước. Không ghi đè thay đổi uncommitted không liên quan. Mặc định **không** auto-commit/push.

## Playwright / UI

Tái sử dụng `tests/e2e/support.mjs` và `npm run test:e2e*`. Verify user workflow, không chỉ page load. Giữ nguyên copy **Đăng Nhập TCS**.

## Persistence / monitor

Session persist để resume. Future `ERROR_MONITOR_AGENT` nối qua `bugReportFromMonitorEvent()`.
