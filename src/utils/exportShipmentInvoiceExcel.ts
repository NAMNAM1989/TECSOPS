...
import type { BuildInvoiceExportOptions } from "../export/builders/buildInvoiceExportPayload";
import {
  buildInvoiceWorkbookFromPayload,
  buildInvoiceWorkbook,
} from "./shipmentInvoiceFill";
...
export type BuildInvoiceOptions = BuildInvoiceExportOptions;
...
export async function buildShipmentInvoiceXlsxBuffer(
...
): Promise<{ buffer: ArrayBuffer; invoiceNo: string; payload: ReturnType<typeof buildInvoiceExportPayload> }> {
  ...
  const { wb, invoiceNo } = buildInvoiceWorkbookFromPayload(ExcelJS, payload);
  const buf = await wb.xlsx.writeBuffer();
  return { buffer: buf as ArrayBuffer, invoiceNo, payload };
}

/** @deprecated Dùng buildShipmentInvoiceXlsxBuffer — giữ tương thích. */
export { buildInvoiceWorkbook, buildInvoiceExportPayload };
