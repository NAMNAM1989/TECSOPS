...
export async function buildTcsH21InvoiceExcelBuffer(doc: H21InvoiceDocument): Promise<ArrayBuffer> {
...
export async function downloadTcsH21InvoiceExcel(
  doc: H21InvoiceDocument,
  awb: string
): Promise<void> {
  const buf = await buildTcsH21InvoiceExcelBuffer(doc);
  downloadXlsxBuffer(buf, h21InvoiceFilename(doc, awb));
}

export function buildTcsH21InvoiceHtml(doc: H21InvoiceDocument): string {
...
export function printTcsH21InvoicePdf(doc: H21InvoiceDocument): boolean {
  const html = buildTcsH21InvoiceHtml(doc);
