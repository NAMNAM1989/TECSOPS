...
export function TcsH21InvoiceModal({
  ...
  const customerPreset = useMemo(
    () => getCustomerH21Preset(customerEntry, "TCS"),
    [customerEntry]
  );

  /**
   * KH đã gán Data H21 tại TCS → bắt buộc chỉ dùng snapshot riêng của KH.
   */
  ...
  const customerPresetPool = boundCustomerCatalog.pool as TcsH21CatalogItem[];
  ...
  const familyCatalog = useMemo(() => {
    ...
    return filterCatalogByH21Family(base, effectiveCargoFamily, 1) as TcsH21CatalogItem[];
        ...
        if (customerPresetPool.length === 0) {
          toast.error(
            customerUnresolvedCount > 0
              ? `Data KH · ${customerEntry?.code ?? "—"}: ${customerUnresolvedCount} SP không còn trong catalog TCS (đổi id / thiếu QC) — gán lại trên Danh mục`
              : "Data KH trống — gán SP trên Danh mục H21 · TCS"
          );
          return;
        }
      }
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {boundToCustomer && customerPresetPool.length > 0 ? (
        ...
        <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-4 py-1.5 text-[11px] text-amber-950">
          ...
          lại list trên Danh mục H21 · TCS.
        </div>
      ) : customerEntry ? (
        <div className="shrink-0 border-b border-ui-border/60 bg-ui-surface-muted/50 px-4 py-1.5 text-[11px] text-ui-text-muted">
          KH {customerEntry.code} chưa up data H21 tại TCS — đang hiện catalog chung.
          Vào Danh mục H21 · TCS → chọn KH → Thay list của KH.
        </div>
      ) : (
        <div className="shrink-0 border-b border-ui-border/60 bg-ui-surface-muted/50 px-4 py-1.5 text-[11px] text-ui-text-muted">
