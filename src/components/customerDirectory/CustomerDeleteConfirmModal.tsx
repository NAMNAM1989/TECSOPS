  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-red-200 bg-white p-5 shadow-2xl">
        <h3 id="delete-customer-title" className="text-lg font-semibold text-red-800">
          Xóa khách hàng?
        </h3>
        <p className="mt-2 text-sm text-apple-secondary">
          Hành động này xóa toàn bộ Shipper, CNEE và tên hàng của{" "}
          <strong className="text-apple-label">{customerName || "khách chưa đặt tên"}</strong>. Không thể hoàn tác.
        </p>
        <label className="mt-4 block text-xs font-semibold text-apple-label">
          Gõ mã khách <span className="font-mono text-red-700">{codeNeedle || "—"}</span> để xác nhận
          <input
            value={typed}
            onChange={(e) => setTyped(e.target.value)}
            autoFocus
            className="mt-1.5 w-full rounded-xl border border-red-200 bg-red-50/50 px-3 py-2 font-mono text-sm uppercase focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-200"
