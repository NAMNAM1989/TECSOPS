/**
 * Chuẩn hóa catalog H21 TCS (shared client/server).
 * Scope cứng: chỉ warehouse TCS.
 */
...
export function tcsH21DescriptionKey(description) {
 ...
 * @param {{ allowQtyRatio?: boolean }} [opts]
 */
 ...
 */
export function normalizeTcsH21CatalogItem(raw, opts = {}) {
...
/**
 * @param {unknown} list
 * @returns {ReturnType<typeof normalizeTcsH21CatalogItem>[]}
 */
export function clampTcsH21Catalog(list) {
  if (!Array.isArray(list)) return [];
  const out = [];
  const seenIds = new Set();
  let i = 0;
  for (const raw of list) {
    const item = normalizeTcsH21CatalogItem(raw, { keepId: true, sortOrder: i });
    if (!item) continue;
 ...
 */
export function findDuplicateTcsH21Descriptions(list, opts = {}) {
  const exceptId = opts.exceptId ? String(opts.exceptId) : "";
  /** @type {Map<string, { description: string, ids: string[] }>} */
  const byKey = new Map();
  if (!Array.isArray(list)) return [];
  for (const raw of list) {
    const item = normalizeTcsH21CatalogItem(raw, { keepId: true });
    if (!item) continue;
 ...
 */
export function findTcsH21DescriptionConflict(list, description, opts = {}) {
  ...
  for (const raw of list) {
    ...
    if (!item) continue;
    if (exceptId && item.id === exceptId) continue;
    if (tcsH21DescriptionKey(item.description) === key) {
 ...
 * @param {unknown} raw
 */
export function normalizeTcsH21InvoiceLine(raw) {
  if (!raw || typeof raw !== "object") return null;
  const o = /** @type {Record<string, unknown>} */ (raw);
 ...
 * @param {unknown} list
 */
export function clampTcsH21InvoiceLines(list) {
  if (!Array.isArray(list)) return [];
  const out = [];
  for (const raw of list) {
    const line = normalizeTcsH21InvoiceLine(raw);
 ...
 */
export function normalizeTcsH21InvoiceDeclaration(raw, seqFallback = 1) {
  if (!raw || typeof raw !== "object") return null;
  const o = /** @type {Record<string, unknown>} */ (raw);
  const lines = clampTcsH21InvoiceLines(o.lines ?? o.items);
 ...
 * @param {unknown} list
 */
export function clampTcsH21InvoiceDeclarations(list) {
  if (!Array.isArray(list)) return [];
  const out = [];
  for (let i = 0; i < list.length; i++) {
    const d = normalizeTcsH21InvoiceDeclaration(list[i], i + 1);
    if (d) out.push(d);
  }
